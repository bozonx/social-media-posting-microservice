...
Текущая реализация `PlatformRegistry` и `PlatformsModule` позволяет легко добавлять новые соцсети. Удаление глобальных настроек платформ (`platforms` в yaml) означает, что все специфичные настройки теперь должны передаваться либо через настройки канала (`channels`), либо быть зашиты в код провайдера. Убедитесь, что это соответствует вашему видению архитектуры.
