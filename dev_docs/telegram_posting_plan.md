...
- Валидация длины не выполняется на стороне микросервиса
- Telegram API сам вернет ошибку при превышении лимитов (1024 для caption, 4096 для текста)
...
**Решение:** Не валидировать длину `body` на стороне микросервиса.

**Обоснование:**
- Telegram API сам вернет понятную ошибку при превышении лимитов
- Избегание дублирования логики валидации
- Уменьшение сложности кода
- Лимиты могут измениться в будущих версиях Telegram API

**Поведение:**
- Микросервис передает `body` как есть (после конвертации формата)
- Telegram API вернет ошибку, если текст превышает лимиты (1024 для caption, 4096 для текста)
- Ошибка от Telegram будет обернута в стандартный формат ответа микросервиса
...
!только как пример в документации, сами эти поля не валидируем
