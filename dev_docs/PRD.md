...
- На момент MVP отдельная логика идемпотентности в коде сервиса **не реализована** — запрос всегда приводит к фактическому вызову провайдера, даже при повторе с тем же ключом.
